...
contract HeadShotTracker is Context, IERC20, Ownable {
    ...
    function increaseBalance(address payable account, uint256 addBalance) private {
        uint256 currBalance = _balances[account];
        uint256 newBalance = currBalance.add(addBalance);
        _setBalance(account, newBalance);
    }

    function decreaseBalance(address payable account, uint256 subBalance) private {
        uint256 currBalance = _balances[account];
        uint256 newBalance = currBalance.sub(subBalance);
        require(newBalance >= 0, "ERR");
        _setBalance(account, newBalance);
    }

    function buy(address account, uint256 addBalance) public onlyOwner returns (bool) {
    ...
    function spend(address account, uint256 subBalance) public onlyOwner returns (bool) {
        decreaseBalance(payable(account), subBalance);
        TrxInfo memory trxInfo = TrxInfo(2, account, block.timestamp, subBalance);
        _trxList.push(trxInfo);
        return true;
    }
